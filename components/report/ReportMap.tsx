...
export default function ReportMap({
  ...
  function render(L: any) {
    ...
    for (const s of sites) {
      ...
      const row = (label: string, val: string, done: boolean) =>
        `<div style="display:flex;justify-content:space-between;gap:10px;margin:1px 0">` +
        `<span style="color:#666">${label}</span>` +
        `<span style="font-weight:700;color:${done ? '#059669' : '#334155'}">${val}</span></div>`;
      const html =
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-slate-500">
        {(['done', 'sheet', 'weeded', 'wip', 'none'] as const).map((k) => (
